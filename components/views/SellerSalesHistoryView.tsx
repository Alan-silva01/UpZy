...
import { ArrowLeft, Search, Edit3, Trash2, Save, X, Loader2, Filter, Calendar } from 'lucide-react';
...
export const SellerSalesHistoryView: React.FC<SellerSalesHistoryViewProps> = ({ user, onBack, initialEditSaleId, onBlockedAction }) => {
  ...
  const handleEdit = (venda: Sale) => {
    ...
    setEditData({
      ...
      data_venda: formatDateTimeLocal(venda.data_venda),
  ...
  const handleSaveEdit = async (vendaId: string) => {
    try {
      // Construir objeto apenas com campos válidos da tabela vendas
      const dataToSave: Record<string, any> = {};

      if (editData.nome_cliente !== undefined) {
        dataToSave.nome_cliente = editData.nome_cliente;
      }
      if (editData.numero_pedido !== undefined) {
        dataToSave.numero_pedido = editData.numero_pedido;
      }
      if (editData.valor !== undefined) {
        dataToSave.valor = editData.valor;
      }
      if (editData.metodo_pagamento !== undefined) {
        dataToSave.metodo_pagamento = editData.metodo_pagamento;
      }
      if (editData.tipo_pagamento !== undefined) {
        dataToSave.tipo_pagamento = editData.tipo_pagamento;
      }
      if (editData.parcelas !== undefined) {
        dataToSave.parcelas = editData.parcelas;
      }
      // Converter datetime-local para ISO string se data_venda estiver presente
      if (editData.data_venda && typeof editData.data_venda === 'string') {
        dataToSave.data_venda = new Date(editData.data_venda).toISOString();
      }
        ...
        <div className="px-1 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`p-2 rounded-full border transition-all ${mostrarFiltros
                ...
                {editingId === venda.id ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      {/* Data da Venda */}
                      <div className="col-span-2 space-y-1">
                        <label className="text-[9px] uppercase tracking-wider text-zinc-500 font-bold ml-1 flex items-center gap-1">
                          <Calendar size={10} />
                          Data da Venda
                        </label>
                        <input
                          type="datetime-local"
                          value={editData.data_venda || ''}
                          onChange={(e) => setEditData({ ...editData, data_venda: e.target.value })}
                          className="w-full bg-zinc-900/50 border border-zinc-700/50 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-indigo-500/50"
                        />
                      </div>
                    </div>
