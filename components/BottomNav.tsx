  ...
  const getNavItems = (): NavItem[] => {
    if (userRole === 'ADMIN') {
      return [
        ...
        { id: Tab.SALES, icon: TrendingUp, label: 'Sales' },
        { id: Tab.ADMIN_SELLERS, icon: UserCheck, label: 'Admin' },
        { id: Tab.SETTINGS, icon: Settings, label: 'Config' },
      ];
    } else {
      return [
        { id: Tab.SELLER_HOME, icon: Home, label: 'Home' },
        { id: 'NEW_SALE_ACTION', icon: PlusCircle, label: 'Novo' },
        { id: Tab.SALES, icon: History, label: 'Histórico' },
      ];
    }
  };
