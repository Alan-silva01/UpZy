...
const PLAN_CONFIG = {
    monthly: {
        value: 119.90, // Updated to match frontend
        cycle: 'MONTHLY',
        installments: 1,
        description: 'Assinatura Mensal UPZY'
    },
    semester: {
        value: 109.90, // Updated to match frontend (6x 109.90 = 659.40)
        installments: 6,
        description: 'Plano Semestral UPZY - 6x sem juros'
    },
    annual: {
        value: 104.90, // Updated to match frontend (12x 104.90 = 1258.80)
        installments: 12,
...
serve(async (req) => {
    ...
    try {
        ...
        console.log('📋 Request data:', JSON.stringify({
            ...
            amountOverride: paymentRequest.amount
        }))
        ...
        // Determine values considering potential overrides from frontend
        let finalInstallmentValue = planConfig.value
        let finalTotalValue = planConfig.value * planConfig.installments

        if (paymentRequest.amount) {
            console.log('⚠️ Using amount override from frontend:', paymentRequest.amount)
            finalTotalValue = paymentRequest.amount
            // Recalculate installment value
            finalInstallmentValue = parseFloat((finalTotalValue / planConfig.installments).toFixed(2))
        }
        ...
        if (paymentRequest.planType === 'monthly') {
            ...
            // For monthly, the value sent is usually the monthly value
            const monthlyValue = paymentRequest.amount || planConfig.value
            ...
            const subscriptionPayload: any = {
                ...
                description: planConfig.description + (paymentRequest.userData.cupom ? ` (Cupom: ${paymentRequest.userData.cupom})` : ''),
                externalReference: userId,
            }
