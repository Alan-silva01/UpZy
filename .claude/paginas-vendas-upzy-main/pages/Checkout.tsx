  ...
  if (success) {
    return (
      <div className="min-h-screen bg-brand-darker flex items-center justify-center p-4">
        <div className="max-w-2xl w-full bg-brand-surface rounded-3xl p-8 border border-white/10 text-center">
          <div className="w-20 h-20 bg-green-500/20 rounded-full flex items-center justify-center mx-auto mb-6">
            <Check size={40} className="text-green-500" />
          </div>

          <h2 className="text-3xl font-bold text-white mb-4">
            {paymentMethod === 'CREDIT_CARD' ? 'Pagamento Confirmado!' : 'Boleto Gerado!'}
          </h2>

          <p className="text-gray-400 mb-8">
            {paymentMethod === 'CREDIT_CARD'
              ? 'Seu pagamento foi processado com sucesso. Você receberá um email de confirmação em breve.'
              : 'Seu boleto foi gerado. Pague até o vencimento para ativar sua assinatura.'}
          </p>
          ...
          {boletoUrl && (
            <a
              ...
              className="inline-block bg-brand-primary hover:bg-brand-primary/90 text-white px-8 py-4 rounded-xl font-bold mb-4 transition-all"
            >
              Abrir Boleto
            </a>
          )}
          ...
          <button
            onClick={onBack}
            className="block w-full bg-gray-700 hover:bg-gray-600 text-white px-8 py-4 rounded-xl font-bold transition-all mt-4"
          >
            Voltar ao Início
          </button>
        </div>
